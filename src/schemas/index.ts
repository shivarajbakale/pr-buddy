...
export const SCHEMAS = {
  ...
  CREATE_PR: {
    ...
    inputSchema: {
      ...
      template: z.enum(['feature', 'bugfix', 'hotfix', 'docs', 'refactor']).optional().describe('PR template type'),
  ...
  LIST_MY_PRS: {
    ...
    inputSchema: {
      ...
      limit: z.number().min(1).max(100).optional().describe('Maximum number of PRs to return'),
    },
  },
  ...
  CHECKOUT_PR_BRANCH: {
    ...
    inputSchema: {
      ...
      createLocal: z.boolean().optional().describe('Create local branch if it doesn\'t exist'),
    },
  },
  ...
  GENERATE_REVIEW_PROMPT: {
    ...
    inputSchema: {
      prNumber: z.number().describe('PR number'),
      reviewType: z.enum(['staff-engineer', 'security', 'performance']).optional().describe('Type of review prompt'),
    },
  },
  ...
  GENERATE_CODE_CHECKLIST: {
    ...
    inputSchema: {
      prNumber: z.number().describe('PR number'),
      includeSecurityChecks: z.boolean().optional().describe('Include security-focused checklist items'),
      includePerformanceChecks: z.boolean().optional().describe('Include performance-focused checklist items'),
    },
  },
  ...
  GET_PR_DIFF_SUMMARY: {
    ...
    inputSchema: {
      prNumber: z.number().describe('PR number'),
      includeFileStats: z.boolean().optional().describe('Include per-file statistics'),
      maxFiles: z.number().min(1).max(100).optional().describe('Maximum number of files to show'),
    },
  },

  GET_PR_STATS: {
    title: 'Get PR Statistics',
    description: 'Get statistics about your merged PRs for different time periods',
    inputSchema: {
      period: z.enum(['day', 'week', 'month']).describe('Time period for statistics'),
      includeRepoBreakdown: z.boolean().optional().describe('Include breakdown by repository'),
      includeDailyBreakdown: z.boolean().optional().describe('Include day-by-day breakdown'),
    },
  },
} as const; 